...
Release 提供 Windows x64 Inno Setup 安装程序、MSI 与便携 ZIP、macOS Apple Silicon (arm64) DMG、Linux x64 DEB 和 RPM，以及每个资产的 SHA-256 校验和。Windows EXE 与 MSI 都会实际安装、启动并卸载；ZIP 会解压并启动；DMG 会挂载并检查应用包；DEB/RPM 会检查包元数据和可执行文件。准确的标签命令与安装说明见仓库 README。
