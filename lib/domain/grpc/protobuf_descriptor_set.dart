...
class ProtobufDescriptorSet {
  ...
  static ProtobufDescriptorSet parse(Uint8List bytes) {
    ...
    while (!reader.isDone) {
      ...
      if (tag >> 3 == 1 && tag & 7 == 2) {
        ...
        final serviceDescriptors = <Uint8List>[];
        while (!file.isDone) {
          ...
          } else if (fileTag >> 3 == 6 && fileTag & 7 == 2) {
            serviceDescriptors.add(file.readLengthDelimited());
          } else {
            // 其余未知字段按 wire 类型跳过。
            file.skip(fileTag & 7);
          }
        ...
        for (final descriptor in serviceDescriptors) {
          final service = _parseService(_WireReader(descriptor), packageName);
  ...
  /// 解析 ServiceDescriptorProto：1=名称、2=方法列表。
  static ProtobufServiceDescriptor? _parseService(
    _WireReader reader,
    String packageName,
  ) {
    String? name;
    ...
    while (!reader.isDone) {
      ...
      if (tag >> 3 == 1 && tag & 7 == 2) {
        ...
        methods.add(_parseMethod(_WireReader(reader.readLengthDelimited())));
      } else {
        reader.skip(tag & 7);
      }
    }
    if (name == null || name.isEmpty) return null;
    final qualified = packageName.isEmpty ? '.$name' : '.$packageName.$name';
    return ProtobufServiceDescriptor(name: qualified, methods: methods);
  }

  /// 解析 MethodDescriptorProto 的名称、输入输出类型及两端流式标记。
  static ProtobufMethodDescriptor _parseMethod(_WireReader reader) {
    String? name;
    String? inputType;
    String? outputType;
    ...
    if (name == null || inputType == null || outputType == null) {
      throw const FormatException('Invalid method descriptor.');
    }
    return ProtobufMethodDescriptor(
      name: name,
