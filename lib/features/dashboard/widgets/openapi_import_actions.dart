...
/// 显示统一的 OpenAPI JSON 导入入口：可直接粘贴，也可先读取本地文件。
...
class _OpenApiImportDialogState extends State<_OpenApiImportDialog> {
  ...
  /// 将输入框内容导入为 OpenAPI 并关闭对话框，随后提示导入结果。
  void _import() {
    final l10n = AppLocalizations.of(context);
    widget.viewModel.importOpenApi(_controller.text);
    // 导入结果消息优先本地化，失败时回退到通用提示。
    final message =
        widget.viewModel.lastActionMessage.localized(l10n) ?? l10n.importFailed;
