...
/// 复制 [value] 到系统剪贴板，并通过 SnackBar 提示 [message]。
///
...
class PanelTitle extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ConstrainedBox(
      // 无副标题时取较矮高度，保证标题栏紧凑
      constraints: BoxConstraints(minHeight: subtitle == null ? 34 : 46),
      child: Row(
        children: [
          Expanded(
            child: Column(
...
class RequestKindPill extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final (label, color) = switch (protocol) {
      ApiRequestProtocol.http => (method, _httpMethodColor(method)),
      ApiRequestProtocol.webSocket => ('WebSocket', AppColors.success),
      ApiRequestProtocol.grpc => ('gRPC', AppColors.primary),
    };
    return Container(
      width: width,
      height: height,
...
Color _httpMethodColor(String method) => switch (method) {
  'GET' => AppColors.methodGet,
  'POST' => AppColors.methodPost,
  'PUT' => AppColors.methodPut,
  'DELETE' => AppColors.methodDelete,
...
class StatusPill extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final color = statusCode >= 400 ? AppColors.danger : AppColors.success;
    return Container(
      height: 24,
      ...
      child: IconButton(
        style:
            IconButton.styleFrom(
              ...
              disabledForegroundColor: AppColors.textFaint.withValues(
                alpha: 0.5,
              ),
